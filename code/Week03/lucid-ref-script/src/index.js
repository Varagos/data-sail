...
/**
 * Once we have it on the blockchain, we wouldn't this here anymore, but we have it since we are also going to deploy the script.
 */
const vestingScript = {
    type: "PlutusV2",
...
async function getReferenceUTxO() {
    const utxos = await lucid.utxosByOutRef([{
        // Since it's in the burn script, and it cant' be spent, it's guaranteed it will always be on that address.
...
/**
 * Here is the interesting part, since we now use the referenceUTxO instead,
 * and we no longer include the script as we did before.
 */
...
/**
 * This is how we deploy the script the first time.
 */
async function onDeploy() {
    const tx = await lucid
        .newTx()
        // We don't need any data, so void here equals the unit, that's how we declare we want to include the script as output
