...
import { Data, Lucid, SpendingValidator, UTxO, applyParamsToScript, fromText, utxoToCore } from 'lucid-cardano';
import { BidDatum, BidDatumType } from './BidSection';

type BidUTxOs = Array<{ utxo: UTxO; datum: BidDatumType }>;

type TokenBids = Array<{ id: string; buyer: string; price: bigint; utxo?: UTxO }>;
...
const AcceptBid = () => {
  ...
  const acceptBid = (tokenId: string, bidId: string) => {
    // Handle bid acceptance logic here
  };
  ...
  /**
   * Applisies the Parameters
   * @param pkh
   * @returns
   */
  const getFinalScript = (tokenPolicyIdHex: string, tokenNameHex: string): SpendingValidator => {
  ...
  async function getBidsForAllTokens(lucid: Lucid) {
    try {
      ...
      const spendingValidators = tokens.map((tokenAssetClass) => {
        // Hash of policy script, always hex length of 56 chars.
        const tokenPolicyId = tokenAssetClass.slice(0, 56);
        return getFinalScript(tokenPolicyId, tokenNameHex);
      });
              ...
              <div className="pl-4 mt-2">
                {token.bids.map((bid, bidIndex) => (
                  <div key={bidIndex} className="flex justify-between py-1">
                    ...
                    <button onClick={() => acceptBid(token.id, bid.id)}>Accept</button>
                  </div>
                ))}
              </div>
            )}
