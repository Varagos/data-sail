...
import { PaymentKeyHash, SpendingValidator, UTxO, getAddressDetails } from 'lucid-cardano';
import { applyParamsToScript, Data } from 'lucid-cardano';
import { useContext, useEffect, useState } from 'react';

const OracleRedeemer = Data.Enum([Data.Literal('Update'), Data.Literal('Delete')]);
type OracleRedeemer = Data.Static<typeof OracleRedeemer>;

/**
 * Code to deploy, update and delete the Oracle
 */
export default function Oracle() {
  ...
  const getOracleNftUtxO = async () => {
    if (lucid && wAddr && oracleAddress) {
      ...
      if (oracWithNftUTxO == undefined || oracWithNftUTxO == oracleWithNftUTxO) return;
      setAppState({
        ...appState,
  ...
  const getFinalScript = async (pkh: PaymentKeyHash): Promise<SpendingValidator | undefined> => {
  ...
  const deployOracle = async () => {
    ...
    const tx = await lucid!
      .newTx()
      .payToContract(oracleAddress, { inline: Data.to(rate, Data.Integer()) }, { [nftAssetClassHex]: 1n })
      .addSignerKey(pkh)
      .complete();
  ...
  const updateOracle = async () => {
    if (wAddr && lucid && nftAssetClassHex && oracleScript && oracleWithNftUTxO && oracleAddress) {
      const pkh: string = getAddressDetails(wAddr).paymentCredential?.hash || '';
  ...
  const deleteOracle = async () => {
    if (wAddr && lucid && nftAssetClassHex && oracleScript && oracleWithNftUTxO && oracleAddress) {
      ...
      alert('You have to deploy the oracle before being able to delete it!');
    }
  };

  const testStuff = async () => {
    // get current wallet
    if (!wAddr || !lucid) throw new Error('no wallet sel3ected');
    const wallet = getAddressDetails(wAddr);
    console.log('address stake credential', wallet.stakeCredential);
    // await lucid.newTx().pay
  };
    ...
    <div className="w-full">
      ...
      <div className="w-full flex flex-row gap-4">
        ...
        <button
          ...
          disabled={!lucid || !wAddr || !nftAssetClassHex || rate === 0n || !!oracleWithNftUTxO}
          className="w-full rounded-lg p-3 text-zinc-50 bg-zinc-800 shadow-[0_5px_0px_0px_rgba(0,0,0,0.6)] disabled:active:translate-y-0 disabled:active:shadow-[0_5px_0px_0px_rgba(0,0,0,0.2)] disabled:bg-zinc-200  disabled:shadow-[0_5px_0px_0px_rgba(0,0,0,0.2)] disabled:text-zinc-600 font-quicksand font-bold active:translate-y-[2px] active:shadow-[0_4px_0px_0px_rgba(0,0,0,0.6)]"
        >
          {' '}
          Deploy Oracle
